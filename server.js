...
io.on('connection', (socket) => {
    ...
    socket.on('entrarJogo', (dados) => {
        jogadores[socket.id] = {
            ...
            nome: dados.nome, classe: dados.classe,
        ...
        io.emit('chatNovaMensagem', { autor: 'SISTEMA', texto: `${dados.nome} entrou!`, cor: '#00ff00' });
    });

    socket.on('entrarCaverna', () => {
        // 1. Defina a nova fase e GERE OS SLIMES!
        if (faseAtual === 0) {
            faseAtual = 1;
            gerarSlimesFase(); // <-- ESTA É A LINHA MÁGICA QUE FALTAVA
        }

        // 2. Notifique todos os clientes sobre a mudança
        io.emit('faseAtualizada', faseAtual);

        // 3. Teleporte o jogador para a entrada da caverna
    ...
    // NOVO: SISTEMA DE RESSUSCITAR / TENTAR DE NOVO
...
server.listen(3001, () => { console.log('Servidor MMORPG a correr!'); });